...
export function CoachesValidationPanel({
  mode = 'page',
  siteId: fixedSiteId,
  accentColor = '#F59E0B',
}: Props) {
  ...
  const pendingUrl = fixedSiteId && mode === 'embedded'
    ? `/coaching/coaches/pending?site_id=${fixedSiteId}`
  ...
  const coachFields = useMemo(() => {
    ...
    return fields.map(f => (f.key === 'status'
      ? { ...f, options: [{ value: 'pending_review', label: 'En attente de validation' }] }
      : f));
  ...
  const openEdit = async (item: PendingCoach) => {
    ...
    try {
      ...
      if (Array.isArray(row.certifications)) detail.certification_ids = (row.certifications as number[]).join(',');
      if (Array.isArray(row.languages)) detail.language_ids = (row.languages as number[]).join(',');
  ...
  return (
    <div className={mode === 'page' ? 'h-full flex flex-col' : 'flex flex-col min-h-0'}>
      {coaches.some(c => c.stale) && (
        <div className={`${mode === 'page' ? 'px-6' : ''} py-2 border-b border-border text-xs text-amber-500`}>
              ...
              { key: 'status', label: 'Statut', render: c => <StatusBadge statut={c.status} /> },
              {
                ...
                render: c => (
                  <div className="flex items-center gap-1">
                    <Button size="sm" variant="ghost" className="h-8 w-8 p-0" disabled={actionId === c.id} onClick={() => openEdit(c)}>
