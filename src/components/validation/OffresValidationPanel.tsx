...
export function OffresValidationPanel({
  mode = 'page',
  siteId: fixedSiteId,
  accentColor = '#2563EB',
}: Props) {
  const scope = useValidationSiteScope('/validation-offres');
  ...
  const pendingUrl = siteId
    ? `/recrutement/offers/pending?site_id=${siteId}&limit=500`
    : '/recrutement/offers/pending?limit=500';
  ...
  const offerFields = useMemo(() => {
    ...
    return fields.map(f => (f.key === 'statut'
      ? { ...f, options: [{ value: 'brouillon', label: 'En attente de validation' }] }
      : f));
  ...
  const offres = useMemo(
    ...
    }).filter(o => !contractFilter || o.type_contrat === contractFilter),
    [data, contractFilter],
  );
  ...
  const openEdit = async (item: PendingOffer) => {
    ...
    try {
      ...
      const sid = Number(row.site_id ?? detail.site_id ?? item.site_id);
      setEditOffer({ item: { ...item, site_id: sid }, detail });
    } catch (err) {
  ...
  const saveOffer = async (raw: Record<string, unknown>) => {
    ...
    try {
      ...
      setEditOffer({ item: editOffer.item, detail: offerDetailFromApi(row) });
    } catch (err) {
  ...
  return (
    <div className={mode === 'page' ? 'h-full flex flex-col' : 'flex flex-col min-h-0'}>
      ...
      <div className={`${mode === 'embedded' ? 'py-2' : 'px-6 py-3'} border-b border-border flex flex-wrap gap-3`}>
        <select
      ...
      {loadingDetail && (
        <div className={`${mode === 'page' ? 'px-6' : ''} py-2 text-sm text-muted-foreground`}>Chargement du détail…</div>
      )}
              ...
              { key: 'statut', label: 'Statut', render: o => <StatusBadge statut={o.statut} /> },
              {
                ...
                render: o => (
                  <div className="flex items-center gap-1">
                    <Button size="sm" variant="ghost" className="h-8 w-8 p-0" title="Voir / modifier" onClick={() => openEdit(o)}>
        ...
        <FormModal
          ...
          footerExtra={(
            <>
              ...
              <Button disabled={!!actionId} style={{ background: '#10B981' }} onClick={() => handlePublish(editOffer.item)}>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => { setRejectModal(null); setRejectMotif(''); }}>Annuler</Button>
              <Button variant="destructive" disabled={actionId === rejectModal.id} onClick={handleReject}>Refuser</Button>
            </div>
          </div>
        </div>
      )}
