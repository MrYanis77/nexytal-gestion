...
export function TrainersValidationPanel({
  mode = 'page',
  siteId: fixedSiteId,
  accentColor = '#0891B2',
}: Props) {
  ...
  const pendingUrl = fixedSiteId && mode === 'embedded'
    ? `/trainer/trainers/pending?site_id=${fixedSiteId}`
              ...
              { key: 'status', label: 'Statut', render: t => <StatusBadge statut={t.status} /> },
              {
                ...
                render: t => (
                  <div className="flex items-center gap-1">
                    <Button size="sm" variant="ghost" className="h-8 w-8 p-0" disabled={actionId === t.id} onClick={() => openEdit(t)}>
