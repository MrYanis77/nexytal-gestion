...
function registerFormationCategoriesRoutes(Router $router): void
{
    $router->get('/api/admin/formation/categories', function () {
        ...
        $db = getDb();
        $stmt = $db->prepare('SELECT * FROM formation_categories ORDER BY sort_order ASC, label ASC');
        $stmt->execute();
    ...
    $router->post('/api/admin/formation/categories', function () {
        ...
        $db = getDb();
        $stmt = $db->prepare('SELECT id FROM formation_categories WHERE slug = :slug LIMIT 1');
        $stmt->bindParam(':slug', $slug, PDO::PARAM_STR);
        $stmt->execute();
        if ($stmt->fetch()) { Response::badRequest('Category slug already exists'); return; }

        $stmt = $db->prepare(
            'INSERT INTO formation_categories (slug, label, description, catalogue_type, sort_order, is_active, created_at)
             VALUES (:slug, :label, :description, :catalogue_type, :sort_order, :is_active, NOW())'
        );
        $stmt->bindParam(':slug', $slug, PDO::PARAM_STR);
        $stmt->bindParam(':label', $data['label'], PDO::PARAM_STR);
        $desc = $data['description'] ?? null;
        $stmt->bindParam(':description', $desc, PDO::PARAM_STR);
        $catType = $data['catalogue_type'] ?? 'all';
        $stmt->bindParam(':catalogue_type', $catType, PDO::PARAM_STR);
        ...
        Audit::log((int) $admin['id'], 1, 'create', 'formation_category', $newId, null, $data);
    ...
    $router->put('/api/admin/formation/categories/{id}', function (array $params) {
        ...
        $stmt = $db->prepare('SELECT * FROM formation_categories WHERE id = :id LIMIT 1');
        ...
        $fields = []; $bind = [];
        foreach (['slug', 'label', 'description', 'catalogue_type', 'sort_order', 'is_active'] as $f) {
        ...
        Audit::log((int) $admin['id'], 1, 'update', 'formation_category', $id, $old, $data);
    ...
    $router->delete('/api/admin/formation/categories/{id}', function (array $params) {
        ...
        Audit::log((int) $admin['id'], 1, 'delete', 'formation_category', $id, $old, null);
        Response::noContent();
    });
}
